 ...
 * 
 * Installer la bibliothèque 'DHT sensor library by Adafruit'
...
#include <ESP8266HTTPClient.h>        // pour utiliser le wifi
#include "config.h"                   // wifi (SSID, password) & serveur
...
void setup() {
  ...
  Serial.println("DHT11 + NodeJS");
  Serial.println("");

  dht.begin();                        // Démarrage du capteur DHT11

  WiFiMulti.addAP(ssid, pass);     // Connexion au réseau wifi
...
void loop() {
  ...
  if (isnan(h) || isnan(t)) {         // test si des valeurs ont été récupérées
    ...
    if((WiFiMulti.run() == WL_CONNECTED)) {   // Si le wifi est connecté
          ...
          http.begin(server);   // connexion au serveur
